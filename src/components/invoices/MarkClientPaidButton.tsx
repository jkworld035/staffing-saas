...
import { markClientInvoicePaid } from "@/app/(dashboard)/invoices/client/actions";

function today(): string {
  return new Date().toISOString().slice(0, 10);
}

export function MarkClientPaidButton({ id }: { id: string }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1.5">
      ...
      <button
        disabled={isPending}
        onClick={() => startTransition(() => markClientInvoicePaid(id, paidDate))}
